...
import {
  ...
  ProjectDaemonStatus,
} from './project-daemon-manager.js'

export class FsProjectDaemonManager implements ProjectDaemonManager {
  constructor(private config: ProjectConfig) {}

  private get paths() {
    return this.config.paths.daemon
  }
  ...
  start(): void {
    ...
    const proc = Bun.spawn(
      [
        ...
        '--daemonized',
        '--project-root',
        this.config.paths.projectRootPath,
      ],
      {
        stdin: 'ignore',
        stdout: 'ignore',
  ...
  enable(): void {
    try {
      unlinkSync(this.paths.disabled)
    } catch {
      /* already enabled */
    }
  }
